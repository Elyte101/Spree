...
| G20 | Guest cart merges into account cart on login | **done** | medium | `lib/stores/cartStore.ts`, `components/providers/cartProvider.tsx` | `initialize()` prefers local (guest) cart over server-provided cart. `mergeGuestCart(serverItems)` added for future server-side cart merge. Since there's no backend cart-per-user table, guest items persist via localStorage (Zustand persist) and survive login via `CartProvider.initialize()`. |
...
| G24 | Admin: in-app chat seller/buyer↔admin ONLY | **done** | medium | `backend/app/api/routes/chat.py`, `components/chat/ChatWidget.tsx`, `app/api/chat/token/route.ts` | Implemented via Stream Chat. Buyer/seller opens support chat from ChatWidget; admin responds via Stream dashboard. Claude AI bot auto-replies when admin is offline. Signature-verified webhook (fail-closed in prod). |
